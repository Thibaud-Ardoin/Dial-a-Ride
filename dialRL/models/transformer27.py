...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask, positions=None):
        ...
        out = self.dropout(
            (x.to(self.device) + positions.to(self.device))
        )
        #self.word_embedding(
...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc_out, src_mask, trg_mask, positions=None):
        ...
        x = self.dropout(self.word_embedding(x.to(self.device)) + positions.to(self.device).permute(0, 2, 1))
