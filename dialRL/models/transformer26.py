...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask, positions=None):
        ...
        # embedded_position = torch.cat([self.position_embedding1(positions[0]), self.position_embedding2(positions[1])], dim=-1).permute(1, 0, 2)

        out = self.dropout(
            (x.to(self.device) + positions.to(self.device)) #embedded_position.to(self.device)) #self.position_embedding(positions.to(self.device))) # positions.to(self.device))
        )
...
class Trans26(nn.Module):
    def __init__(
        ...
        self.boxh, self.boxw = abs(self.extremas[2] - self.extremas[0]) / self.src_vocab_size, abs(self.extremas[3] - self.extremas[1]) / self.src_vocab_size
    ...
    def fourier_feature(self, coordonates):
        ...
        if x.shape[1] == 1:
            transB = transB[0].unsqueeze(0)
        # ic(transB)
        # ic(x)
        x_proj = x.matmul(transB)
        # ic(x_proj)
        out = self.quinconx(torch.sin(x_proj), torch.cos(x_proj))
        # ic(out)
        # exit()
        return out
    ...
    def env_encoding(self, src):
        ...
        bsz = w[0].shape[-1]
        world_emb = self.fourier_feature(w[0].unsqueeze(-1))

        # 10 * target id + 100 * state
        targets_emb = [self.fourier_feature(torch.stack([target[0], target[1]+2]).permute(1,0)) for target in ts]
        # 1000 * driver id
        drivers_emb = [self.fourier_feature(driver[0].unsqueeze(-1)) for driver in ds]

        final_emb = torch.stack([world_emb] + targets_emb + drivers_emb).long().to(self.device)
    ...
    def bidim2int(self, coordonate):
        ...
        return torch.stack([h, w]).long().to(self.device)
