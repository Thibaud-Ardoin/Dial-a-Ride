...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask, positions=None):
        ...
        out = self.dropout(
            (self.word_embedding(x.to(self.device)) + self.position_embedding(positions.to(self.device))) #self.position_embedding(positions.to(self.device)))
        )
...
class Trans2(nn.Module):
    ...
    def env_encoding(self, src):
        ...
        final_emb = torch.stack([world_emb] + targets_emb + drivers_emb).long()
