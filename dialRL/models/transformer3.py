...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask, positions=None):
        ...
        out = self.dropout(
            (self.word_embedding(x.to(self.device)) + embedded_position.to(self.device)) #self.position_embedding(positions.to(self.device))) # positions.to(self.device))
        )
...
class Trans3(nn.Module):
    ...
    def env_encoding(self, src):
        ...
        targets_emb = [target[0] * 10 + (target[1]+2) * 100 for target in ts]
        # 1000 * driver id
        drivers_emb = [driver[0]*1000 for driver in ds]
    ...
    def positional_encoding(self, position):
        ...
        targets = [self.bidim2int(pos) for pos in position[1]]
